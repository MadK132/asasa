...
При локальном запуске нужно подключать:

- `theme` в `/opt/keycloak/themes`
- `realm-config` в `/opt/keycloak/data/import`

Это важно, потому что:

- `theme` подключает локальную тему Keycloak
- `realm-config` нужен для импорта realm при старте
